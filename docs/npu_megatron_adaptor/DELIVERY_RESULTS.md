...
## 7. dense mxfp8 完整对比(精度 + 算子级加速 + 模型级饱和,2026-06-22）

dense 三件套补齐:精度 parity（§2:corr 0.999/0.977 + 100步收敛 reward 0.231/0.235 齐平）+ 下面的算子级与模型级。

### 7.1 dense Linear mxfp8 vs bf16 算子加速比（hidden 2048,attn-proj 2048→2048,稳态权重缓存）
| M(token) | 前向 加速比 | 训练(fwd+bwd) 加速比 |
|---|---|---|
| 256–2048 | 0.23–0.28x | 0.43x |
| 4096 | 0.52x | 0.50x |
| 8192 | **1.08x（前向拐点）** | 0.97x |
| 16384 | **1.55x** | **1.43x** |
| 32768 | 1.50x | 1.42x |

- dense Linear 饱和后 **~1.5x（前向）/ ~1.42x（训练）**;bf16 峰值 ~420 TFLOPs,mxfp8 ~640 TFLOPs。
- **拐点 M≈8192**(比 MoE 专家的 ~1024–2048 高):单个 Linear 的 mxfp8 量化有 ~0.16ms 固定开销底,需更大 M 才摊开。
- 与 §6.1 MoE GroupedLinear 一致的规律:mxfp8 收益"饱和依赖",小 M 反而更慢。

### 7.2 dense Qwen3-0.6B 模型级饱和成对（GRPO,micro=32,4 卡 EP1×TP2,无 offload）
| 指标 | bf16 | mxfp8 |
|---|---|---|
| corr | 0.9992 | 0.9754 |
| MFU | ~0.155 | ~0.152 |
| 吞吐 tok/s | ~1395 | ~1465(**+5%**) |

- 精度 parity ✓(mxfp8 corr 下降是量化预期);模型级仅 **+5%、MFU 基本持平**。
- 原因:**0.6B 太小(hidden=1024,K 小,mxfp8 单算子收益本就弱)+ 非-GEMM 时间稀释**。算子级(hidden 2048)能到 1.5x,但 0.6B 模型吃不到。
- **结论:再次印证 mxfp8 端到端收益需要"大模型 / 大 K / 饱和"** —— 这正是目标 Qwen3-30B MoE(专家 GEMM 大、可饱和)预期收益显著高于 0.6B 的原因。

## 8. 全 30B MoE(48 层）精度/性能预判（基于已有数据外推,⚠️ 待 card5 修复后实测确认)

> 全 48 层因 .18 card5 坏卡未能端到端实测;以下是基于算子级 + 1/8 层 MoE + dense 的合理外推,非测量值。

### 精度:预计 parity,无大差异
- 依据:1 层 corr mxfp8 0.978-0.981 → 8 层 0.993,**层数增加未见恶化趋势**;mxfp8 是混合精度,**主权重/残差流/norm/优化器态全程高精度**,量化误差不跨层累积(fp8 训练能用于百亿级全深度模型的根本原因)。
- 预判:48 层 corr 仍在 **~0.97-0.99**,长程收敛追平 bf16。风险:corr 比 8 层再小幅下移,但非"大差异"。

### 性能:饱和时训练段 ~1.2x,端到端 +10-20%,强依赖专家饱和
- 依据:专家 GroupedLinear 饱和(tok/expert≥2048)训练 **1.24x**;48 层 GEMM 主导(专家占 27B/30B,embedding/lm_head 可忽略)→ 比 8 层代理稀释少。
- **关键前提:专家必须喂饱**(tok/expert≥2048 → 每卡每步 ≥ ~32768 token)。RL batch 不够大 → 专家欠饱和 → 落拐点左侧、收益打折甚至变慢。
- 端到端 RL 吞吐再被生成段稀释 → 预计 **+10-20%**(取决于 rollout 占比)。

### 待实测项(card5 修复 / 换 .54 类节点后)
全 30B 8 卡 EP4×TP2:48 层 corr(bf16 vs mxfp8)+ 饱和 batch 下的训练段/端到端吞吐与 MFU。
